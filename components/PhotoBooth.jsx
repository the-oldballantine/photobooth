...
function CameraPanel({ label, videoRef, ready, error, mine, pulsing, liveUrl, useVideo, remoteVideoRef }) {
  return (
    <div className={pulsing ? "pb-pulse-ring" : ""} style={styles.camPanel}>
      ...
      <div style={styles.camFrame}>
        {mine ? (
          ...
          <video ref={remoteVideoRef} autoPlay playsInline style={styles.camVideo} />
        ) : liveUrl ? (
          <img src={liveUrl} alt={label} style={styles.camVideo} />
        ) : (
          <div style={styles.camWaiting}>
            {ready ? (
              <span style={styles.waitPulseWrap}>
                <span className="pb-blink" style={styles.waitDot} />
                <span style={{ color: "rgba(255,246,241,0.6)", fontSize: 12 }}>connecting camera...</span>
              </span>
            ) : (
              <span style={styles.waitPulseWrap}>
                <span className="pb-blink" style={styles.waitDot} />
...
export default function App() {
  ...
  // The "you" <video> element gets unmounted and re-created every time the
  // screen changes (lobby -> session are separate render trees). A plain
  // ref only attaches the stream once, so after the first screen change the
  // new <video> node sits empty (videoWidth stays 0), captureFrame() starts
  // returning null, and that side's shot never lands — the whole flow stalls.
  // This callback ref re-attaches the live stream every single time a new
  // video node mounts, so "you" never goes blank.
  const attachOwnVideo = useCallback((node) => {
  ...
  const waitForIceGatheringComplete = useCallback((pc) => {
    ...
    return new Promise((resolve) => {
      ...
      // Safety timeout in case gathering stalls on a tricky network.
  ...
  // Host: once both people are in the room and the camera is live, create
  // an offer and publish it to the room for the partner to pick up.
  useEffect(() => {
    if (role !== "host" || !roomCode || !bothJoined || !camReady || createdOfferRef.current) return;
    ...
    (async () => {
      try {
        ...
        await roomAction(roomCode, "webrtc-offer", { sdp: JSON.stringify(pc.localDescription) });
      } catch (e) {}
    })();
  }, [role, roomCode, bothJoined, camReady, createPeerConnection, waitForIceGatheringComplete]);

  // Host: apply the partner's answer once it shows up.
  useEffect(() => {
    if (role !== "host" || !pcRef.current || !room?.webrtcAnswer || appliedAnswerRef.current) return;
    appliedAnswerRef.current = true;
    (async () => {
      try {
        const answer = JSON.parse(room.webrtcAnswer);
        await pcRef.current.setRemoteDescription(new RTCSessionDescription(answer));
      } catch (e) {}
    })();
  }, [role, room?.webrtcAnswer]);

  // Partner: once the host's offer shows up, answer it.
  useEffect(() => {
    if (role !== "partner" || !roomCode || !camReady || !room?.webrtcOffer || answeredRef.current) return;
    answeredRef.current = true;
    (async () => {
      try {
        const offer = JSON.parse(room.webrtcOffer);
        const pc = createPeerConnection();
        pcRef.current = pc;
        ...
        await waitForIceGatheringComplete(pc);
        await roomAction(roomCode, "webrtc-answer", { sdp: JSON.stringify(pc.localDescription) });
      } catch (e) {}
    })();
  }, [role, roomCode, camReady, room?.webrtcOffer, createPeerConnection, waitForIceGatheringComplete]);

  // Attach the incoming remote stream to whichever <video> element is
  // currently mounted (lobby vs session render separate instances).
  useEffect(() => {
  ...
  const captureFrame = useCallback(() => {
    ...
    const contrast = 1.0;
    const brightness = 55; // -255..255
    const contrastOffset = 128 * (1 - contrast);
    for (let i = 0; i < px.length; i += 4) {
      // Luminance-weighted grayscale.
      let gray = px[i] * 0.299 + px[i + 1] * 0.587 + px[i + 2] * 0.114;
      // Light, airy tone instead of a heavy punchy contrast.
      gray = gray * contrast + contrastOffset + brightness;
    ...
    // Barely-there vignette — just a whisper of edge falloff.
    ctx.save();
    ...
    // Visible film grain for texture.
    ctx.save();
    ...
    // Faint scratch/light-leak streaks for extra filmic character.
    ctx.save();
  ...
  // Periodically push a low-res snapshot of our own camera so the partner
  // sees a near-live feed instead of a static "connected" label. This is not
  // true video streaming — it's a cheap frame-polling stand-in, updated
  // roughly once a second, which is enough for a lobby/session preview.
  useEffect(() => {
    if (!roomCode || !role) return;
    ...
    uploadFrame();
    const liveRef = setInterval(uploadFrame, 1000);
    return () => clearInterval(liveRef);
  }, [roomCode, role, screen, captureFrame]);
    ...
    <div style={styles.page}>
      ...
      {screen === "lobby" && room && (
        <div className="pb-fadein pb-bento pb-lobby">
          ...
          <Tile area="you" tone="dark">
            <CameraPanel label="you" videoRef={attachOwnVideo} error={camError} mine />
          </Tile>
          <Tile area="partner" tone="dark">
            <CameraPanel
              ...
              ready={bothJoined}
              mine={false}
              liveUrl={role === "host" ? room.livePartner : room.liveHost}
              useVideo={webrtcConnected}
      ...
      {screen === "session" && room && (
        <div className="pb-fadein pb-bento pb-session" style={{ position: "relative" }}>
          ...
          <Tile area="you" tone="dark">
            <CameraPanel label="you" videoRef={attachOwnVideo} error={camError} mine pulsing={inCountdown} />
          </Tile>

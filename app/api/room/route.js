...
async function readRoom(code) {
  ...
  return {
    ...
    shots,
    liveHost: data.live_host || null,
    livePartner: data.live_partner || null,
    webrtcOffer: data.webrtc_offer || null,
    webrtcAnswer: data.webrtc_answer || null,
  };
}
...
export async function POST(request) {
  try {
    ...
    if (action === "live") {
      // Lightweight periodic preview frame — atomic per-role field, no
      // full room read on the response since the UI just needs ok/fail.
      const { role, dataUrl } = body;
      if (!role) {
        return Response.json({ error: "role is required" }, { status: 400 });
      }
      await kv.hset(key, { [`live_${role}`]: dataUrl || "" });
      await kv.expire(key, ROOM_TTL_SECONDS);
    ...
    if (action === "webrtc-offer") {
      const { sdp } = body;
      if (!sdp) return Response.json({ error: "sdp is required" }, { status: 400 });
      await kv.hset(key, { webrtc_offer: sdp });
      await kv.expire(key, ROOM_TTL_SECONDS);
      return Response.json({ ok: true });
    }

    if (action === "webrtc-answer") {
      const { sdp } = body;
      if (!sdp) return Response.json({ error: "sdp is required" }, { status: 400 });
      await kv.hset(key, { webrtc_answer: sdp });
      await kv.expire(key, ROOM_TTL_SECONDS);
      return Response.json({ ok: true });
    }
